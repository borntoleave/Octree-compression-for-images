...
//	cerr <<l+1<<" th pixel\t"<<T.clr_cnt<<"colors\t";
	if(T.clr_cnt>T.limit)
		{flag=0;
		T.find();	//find the lowest node that can be combined	
//		cerr<<"del "<<T.lowest->childcount<<"children at level_"<<T.lowest->level<<"\t";
